...
if st.session_state.step == 1:
    ...
    name = st.text_input('이름', key='name_input', value=st.session_state.get('saved_name', ''))
    age = st.number_input('나이', min_value=1, max_value=100, value=st.session_state.get('saved_age', 20), key='age_input')
    
    if st.button('다음 →'):
        if name: # 이름이 입력되었는지 확인
            # [수정] 데이터를 세션 스테이트에 저장해야 Step 3에서 보입니다.
            st.session_state.saved_name = name
            st.session_state.saved_age = age
            st.session_state.step = 2
            st.rerun() 
        else:
            # [오류 해결] 이 else의 들여쓰기를 위의 if와 맞췄습니다.
...
# ---- Step 2: 관심 분야 ----
    ...
    interests = st.multiselect(
        ...
        default=st.session_state.get('saved_interests', []),
        key='interests_input'
    )
...
# ---- Step 3: 확인 ----
    ...
    with col2:
        if st.button('✅ 제출'):
            ...
            # 잠시 후 페이지 갱신을 위해 rerun 호출 가능 (선택 사항)