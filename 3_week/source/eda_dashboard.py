...
# [수정] '2026-01=01'을 '2026-01-01'로 변경했습니다.
df = pd.DataFrame({
...
# ---- 데이터 필터링 ----
# 날짜 기준으로 데이터를 자른 뒤 카테고리 필터를 적용합니다.
filtered_df = df.head(data_range).copy() # .copy()를 써주는 것이 안전합니다.
...
with tab1:
    ...
    if not filtered_df.empty:
        kpi1.metric('총 매출', f"₩{filtered_df['매출'].sum():,}만", f"{filtered_df['매출'].mean():.0f} (평균)")
        kpi2.metric('총 고객수', f"{filtered_df['고객수'].sum():,}명", f"{filtered_df['고객수'].mean():.0f} (평균)")
        kpi3.metric('평균 전환율', f"{filtered_df['전환율'].mean():.2%}", f"{filtered_df['전환율'].std():.2%} (표준편차)")
    else:
